...
As illustrated in figure below, the `top.vhd` architecture contains the PTRNG, including the [digital noise source](#digital-noise-source) (more information on its configuration can be found [here](#digital-noise-configuration)).

![Top architecture](/assets/images/top.png)

A communication channel is enabled from the PC to the PTRNG with the help of an UART, and a simple interpreter for read/write commands that performs transactions on the [register map](#register-map-configuration).

The register map helps to:
- send control and configurations to the PTRNG,
- read statuses from the PTRNG,
- and read random data generated by the PTRNG in the FIFO.
...
It is essential to provide the synthesis tool with specific constraints to enable combinatorial loops. Additionally, the place and route tool requires physical constraints to ensure reproductibility and signal integrity, particularly to minimize crosstalk from other parts of the design via cells or nets.
...
As instance, the testbench simulation produces the following output:
...
Thanks to the `ghdl` simulator, all testbenches store their waveforms in `waves.vcd` files. These waveform files can be visualized using tools such as [GTKWave](https://sourceforge.net/projects/gtkwave) or [Wavedrom](https://wavedrom.com), among others.

![COSO testbench waves](/assets/images/cosowaves.png)

## Compile for FPGA

The VHDL provided in the `hardware/hdl` directory is target-agnostic and can be synthesized for all FPGA vendors (or ASICs). In contrast, the ring-oscillator implementations are target-specific. As instance, the `target/common/xilinx` directory contains a dedicated `ring.vhd` implementation for Xilinx FPGAs.

### FPGA targets

Currently, OpenTRNG supports the [Digilent Arty A7](https://digilent.com/reference/programmable-logic/arty-a7/start) board (35T and 100T), which is based on the [Xilinx Artix-7](https://www.xilinx.com/products/silicon-devices/fpga/artix-7.html) FPGA.

![image-center](/assets/images/arty7.png)

If you need to support other hardware targets, please follow these instructions:
...
Some part of the design are automatically generated with scripts (eg. [register map](#register-map-configuration)) or configured via scripts (such as for [digital noise](#digital-noise-configuration)).
